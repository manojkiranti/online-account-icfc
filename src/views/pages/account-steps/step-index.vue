<template>
  <div class="steps-wrapper">
              ...
              <template slot="footer" slot-scope="props" >
                ...
                <div class="wizard-footer-right" v-if="hasAlreadyAccount == 'no'">
                  <b-button
                    ...
                    >  <b-spinner small v-if="draftLoading"></b-spinner> Save As Draft</b-button>
                  <button
                    v-if="!props.isLastStep"
                    @click="props.nextTab()"
    ...
    <b-modal ref="modal-draft" title="Success" ok-only>

      <p class="my-4">Your information is saved. Please use reference code send to your email to continue</p>
    </b-modal>
  </div>
</template>
<script>
...
export default {
  ...
  created() {
    ...
    bus.$on('getStepData', (data) => {
      ...
      this.refereceCode = data.refrenceNo
    })
  ...
  methods: {
    saveAsDraft() {
      ...
      const operation = this.response(stepApi.saveDraft(this.refereceCode));
      operation
        .then((data) => {
          if (operation.isFulfilled()) {
            // console.log('draft',data);
            this.draftLoading = false;
            this.$refs['modal-draft'].show()
          }
        })
        .catch((err) => {
